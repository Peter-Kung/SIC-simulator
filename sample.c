...
do {
    ...
    if (strcmp( cmd, s_command[0]) == 0){//處理檔名
         tmp = cLOAD; 
...
int rd_text () {//寫入 text record 進 memory
    char tmp[7];
    int i, j, l, s;
    ...
    for (i=1, j=0; i<7; i++, j++)  tmp[j] = o_line[i];//取得開始位置
    tmp[j] = '\0';
    sscanf( tmp, "%x", &s );

    for (i=7, j=0; i<9; i++, j++)  tmp[j] = o_line[i];//取得長度
    tmp[j] = '\0';
    sscanf( tmp, "%x", &l );
    
    l = 9 + l * 2;
    int len = strlen(o_line);
    for (i=9, j = (s - start_add) * 2 + start_add; i < len; i++, j++)//寫入 memory
        if(o_line[i] != '\n')
    	    memory[j] = o_line[i];
...
void rd_end () {//跳到第一個可執行 object code
    char tmp[7];
    int i, j;
...
void s_load () {//load object code to memory

    if(loaded){puts("Program is not released yet!\n");}
    else{
        rd_header();//獲得 start address 與 program length
        while(rd_text());//寫入 memory
        rd_end();
    }
}
...
void s_unload () {//釋放所有 data structure
